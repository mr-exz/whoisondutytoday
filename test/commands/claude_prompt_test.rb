...
class ClaudePromptTest < ActiveSupport::TestCase
  test "download_and_save_file returns unique filename with file ID" do
    ...
    begin
      ...
      WhoIsOnDutyTodaySlackBotModule::Commands::ClaudePrompt.stub(:download_with_redirects, mock_http_response) do
        ...
        assert_equal 'image_F09V5KEH1CK.png', result
  ...
  test "download_and_save_file handles files without extension" do
    temp_dir = Dir.mktmpdir('claude_prompt_test')
    ...
    begin
      ...
      WhoIsOnDutyTodaySlackBotModule::Commands::ClaudePrompt.stub(:download_with_redirects, mock_http_response) do
        ...
        assert_equal 'README_F123', result

        expected_filepath = File.join(temp_dir, 'README_F123')
        assert File.exist?(expected_filepath)
      end
